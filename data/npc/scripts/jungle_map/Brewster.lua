...
local stoneKeyword = keywordHandler:addKeyword({'adventurer stone'}, StdModule.say, {npcHandler = npcHandler, text = 'Ah, you want to replace your adventurer\'s stone for free?'}, function(player) return player:getStorageValue(GameConfig.AdventurersGuild.FreeStone.Brewster) < os.time() end)
	stoneKeyword:addChildKeyword({'yes'}, StdModule.say, {npcHandler = npcHandler, text = 'Here you are. Take care.', reset = true}, nil, function(player) player:addItem(18559, 1) player:setStorageValue(Storage.AdventurersGuild.FreeStone.Brewster, os.time() + 86400) end)
...
local stoneKeyword = keywordHandler:addKeyword({'adventurer stone'}, StdModule.say, {npcHandler = npcHandler, text = 'Ah, you want to replace your adventurer\'s stone for 30 gold?'})
	stoneKeyword:addChildKeyword({'yes'}, StdModule.say, {npcHandler = npcHandler, text = 'Here you are. Take care.', reset = true}, function(player) return player:getMoney() >= 30 end, function(player) player:removeMoney(30) player:addItem(18559, 1) end)
...
-- local stakeKeyword = keywordHandler:addKeyword({'stake'}, StdModule.say, {npcHandler = npcHandler, text = 'Yes, I have been informed and haven\'t yet drunk enough to forget, so you can count yourself lucky. Are you prepared to <hicks> receive my line of the prayer?'}, function(player) return player:getStorageValue(Storage.FriendsandTraders.TheBlessedStake) == 9 end)
	-- stakeKeyword:addChildKeyword({'yes'}, StdModule.say, {npcHandler = npcHandler, text = 'So receive my prayer: \'Your hand shall be guided - your feet shall walk in <hicks> harmony\'. Now, take your stake to Tyrias in <hicks> Liberty Bay for the next line of the prayer. I shall let him know what he is to do.', reset = true}, nil,
		-- function(player) player:setStorageValue(GameConfig.FriendsandTraders.TheBlessedStake, 10) player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE) end
	-- )
