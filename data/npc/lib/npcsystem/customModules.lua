...
function StdModule.rookgaardHints(cid, message, keywords, parameters, node)
	...
	local hintId = player:getStorageValue(GameConfig.RookgaardHints)
	npcHandler:say(hints[hintId], cid)
	if hintId >= #hints then
		player:setStorageValue(GameConfig.RookgaardHints, -1)
	else
		player:setStorageValue(GameConfig.RookgaardHints, hintId + 1)
	end
	return true
end
